...
def Ptf_target_optimization_Gurobi(E_return, E_cov, K, Nbr_PTF, bounds):
...
def Ptf_target_optimization_W_Rf_Gurobi(E_return, E_cov, Expected_Risk_free, K, Nbr_PTF, bounds):
...
def boostrap(Data,Nsim):
    np.random.seed(123)
    block_size=np.mean(optimal_block_length(Data)['stationary'])
...
#Resample Efficient Frontrier with no risk
def resample_no_risk(E_return, E_cov,Resample,K, Nbr_PTF,path_regression,bounds): #With Stationnary block resampling
    ...
    for j in range(0,sims):
        Data=Resample[j]
        #Estimation Return
        RE_return=return_est(Data,path_regression)
        RE_cov=Ledoit_wolf(Data)
...
#Resample Efficient Frontrier with  risk free asset
def resample_with_risk(E_return, E_cov,Resample,Expected_Risk_free, K, Nbr_PTF,path_regression, bounds): #With Stationnary block resampling
    Weigth_names = np.concatenate((np.array(['Risk_free']), np.array(E_return.index)))
...
#Tangent for Resample Efficient Frontrier
...
def plot_Resample_ef_Gurobi(E_return,E_cov,result1,result2,Expected_Risk_free,show_cml=False, show_gmv=False):
    """Function to plot Resample Efficient Frontier"""
